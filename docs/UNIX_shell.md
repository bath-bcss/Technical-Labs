...
This will be where you write commands. The dollar symbol tolls you you are not the root user, but 
...
in your "Home directory". In UNIX you can always find the home directory as `~` represents the 
current users home directory, thats why it is in the prompt, thats your current directory.
...
1. Internal commands - certain commands fall into the remit of the operating system, like managing 
   the file system, these commands are managed by the OS 'kernal' and dont rely on any executables.
2. Included commands - some binaries come included with your operating syste and are considered part 
   of the operating system.
3. External commands - executables the user has installed

If the command comes in the firse category then the shell hands the job straight off to the kernal 
to handle.
But otherwise the shell has to find the command you want to run, so it goes and looks in special 
directories. These directories are spcified by the PATH variables, which will be covered later.
...
One common style type of parameter are "flags". These look like the `-a` and `-m` above and 
generally alter the commands behaviour in some way.

Different commands take different parameters in different orders, you can normally find the list of 
parameters for any command on the "man" page, by running `man <command name>` and then exit the page 
by pressing the `q` key.

File management in UNIX
-----------------------

One of the first things you may want to do is manage files and directorys. All of which can be done 
with a UNIX shell.
...
_*Warning*_ In UNIX there is no "trash" directory where delted files go. When you delete a file it 
is gone. 
...
![EchoCommand](Assets/UNIX_Shell/echoCommand.png)
...
test ececutables you make yourself.
...
The `cat` command reads a file and outputs it to the standard out.
...
Here the `ip` command lists lots of networking information, but I only want the line about the ip 
address starting "127." so I use the `grep` command that lets you search the standard in for certain 
strings.
...
if you dont understand a command never be arfraid to look into the man pages for it!
