...
theres a whole bunch of tools that make up a category the [MIT Missing semester 
series](https://missing.csail.mit.edu/) call "metaprogramming". That is, 
software for the purpose of writing software.
...
These tools can be loosely grouped into four categories, but theres lots of 
overlap and tools that can me in many or none of the categories. It's an 
...
their own suite of tool. Then there are a selection of more general tools that 
...
automation 
tools](https://en.wikipedia.org/wiki/List_of_build_automation_software). 
...
In this section when I say "build" I mean it in the broadest sense of taking 
some input you've made and making an output. Building could be compiling a large 
code base into a binary, or generating a PDF from LaTeX code.

For large projects that involve a variety of technologies building the project 
can take many time consuming steps. In addition, if you want collaborators on 
...
As said earlier, there are a lot of build automation systems, which one is best 
for a project depends on the technologies being used, the size of the project, 
and personal taste. Regardless, most build systems work off a system of 
*dependencies*, *targets* and *rules* dictating how to go from dependencies to 
targets. 

When you tell the build system to you want a particular target it will find all 
the transitive dependencies of that target and apply the rules to produce all 
the intermediate targets until the final target has been produced. However, if 
any of the intermediate targets have been build previously and haven't changed 
the build system will not make it again. Which can often lead to massive time 
saves in big projects.
...
In this example we're making a report in LaTeX, this report needs a graph from a 
set of data points. Thus, that graph is a *dependency* of the report *target*. 
We can also define rules for how to make the report given the graph, and how to 
make the graph given the set of data points.
...
Each item in a `MakeFile` is a *rule*, first comes a *target*, then everything 
after the colon make up the dependencies for this target. Lastly the lines below 
the item are the commands needed to create the target from the dependencies. 
...
project. Continuous development is a subset of those tasks which deploy the 
project.

### Example

Here we're going to look at the [repository](https://github.com/AlfGalf/jekyllWebsite) for my website
...
of tasks you can automate with GitHub action, or with the array of similar 
tools.
